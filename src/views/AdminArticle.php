...
<aside class="hidden sm:flex sm:flex-col fixed top-0 left-0 h-full w-20 z-50">
  ...
  <img src="../../public/assets/images/Neon Green and Black Graffiti Urban Grunge Logo.png" alt="Banner Image" class="w-full h-full object-cover" />
    ...
    <div class="flex-grow flex flex-col justify-between text-gray-500 bg-gray-800">
      ...
      <span class="sr-only">Dashboard</span>
        ...
        <a href="AdminArticle.php" class="inline-flex items-center justify-center py-3 text-purple-600 bg-white rounded-lg">
 ...
 <?php  }else{ ?>
  ...
  <div class="flex-grow text-gray-800 lg:ml-20 md:ml-20 sm:ml-20">
    ...
    <div class="flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        function afficheArtcile($conn,$idUser){
  ...
  <div class="relative">
    ...
    if ($row[5]==True) {
      echo '<div class="absolute top-3 left-3 bg-green-500 text-white rounded-full p-1 shadow-lg flex items-center justify-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
        </svg>
      </div>';
    } else {
      echo '<div class="absolute top-3 left-3 bg-red-500 text-white rounded-full p-1 shadow-lg flex items-center justify-center">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        if(isset($_POST['validate'])){
          ...
          $image_path = $target_dir . $unique_identifier . '.' . $imageFileType;
            ...
            if (move_uploaded_file($_FILES["image_Article"]["tmp_name"], $image_path)) {
              ...
              $sql = "UPDATE Article SET Titre_Article='$titre_Article', Contenu_Article='$Contenu_Article' WHERE Id_Article = '$id_Article'";
              mysqli_query($conn,$sql);
              echo "<script>window.location.href = window.location.href;</script>";
            }
